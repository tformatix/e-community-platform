...
class LoginViewModel(_application: ECommunityApplication) : LegacyLoadingStateViewModel(_application) {
    ...
    fun login(_email: String, _password: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                mState.emit(LegacyLoadingState(LegacyLoadingState.State.RUNNING, LoadingStateId.LOGIN.ordinal))
                ...
                mState.emit(LegacyLoadingState(LegacyLoadingState.State.SUCCESS, LoadingStateId.LOGIN.ordinal))
            } catch (_exc: Exception){
                // an error occurred
                mState.emit(LegacyLoadingState(LegacyLoadingState.State.FAILED, LoadingStateId.LOGIN.ordinal, _exc))
            }
        }
    }
    ...
    fun resendConfirmationEmail(_email: String, _password: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                mState.emit(LegacyLoadingState(LegacyLoadingState.State.RUNNING, LoadingStateId.RESEND_CONFIRMATION_EMAIL.ordinal))

                val authApi = AuthApi(Constants.HTTP_BASE_URL_CLOUD)
                authApi.authResendConfirmationEmailPost(LoginMemberModel(_email, _password))

                mState.emit(LegacyLoadingState(LegacyLoadingState.State.SUCCESS, LoadingStateId.RESEND_CONFIRMATION_EMAIL.ordinal))
            } catch (_exc: Exception){
                // an error occurred
                mState.emit(LegacyLoadingState(LegacyLoadingState.State.FAILED, LoadingStateId.RESEND_CONFIRMATION_EMAIL.ordinal, _exc))
            }
        }
    }
